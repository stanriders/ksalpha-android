...
init -1 python:
    ...
    # [ 
    android_build = True  # !! ONLY WHEN BUILDING ANDROID VERSION !! #
    # ]
...
init 1:
    ...
    $ config.searchpath.append('game/archived') #$ config.searchpath.append('archived')
...
#    bgright = Position(xanchor=0.5, xpos=0.6)
    ...
    def define_characters(): # now a function for i18n purposes
        ...
        store.narrator = Character(' ', what_prefix="", what_suffix="", show_function=say_wrapper,
                                         what_yoffset=20) # hacks make me cry ;c [str]
...
#    music_ease = "bgm/Ease.ogg"
    ...
    #video mapping
    #vid_tca1 = "video/tc_act1.mkv" # no? i dunno [str]
    vid_op = "video/op_1.mp4"
    vid_disc = "video/disc.mp4"
    vid_dream = "video/nightmare.mp4"
    

    # CONFIG

    # [ i dont really care what it does, i'm just making sure that everything is same as in 6.10
    ...
    if android_build:
        config.mouse = {"default": [("ui/bt-blank.png",1,0)]}
    else:
    ...
    config.window_icon = None # we dont really need it on android [str] #"ui/icon.png"
    ...
    # i feel stupid [str]
    style.notmm_button.background = None
    style.notmm_button.xminimum = 0
    style.notmm_button_text.xalign = 0
    style.notmm_button_text.font = titlefont
    style.notmm_button_text.color = "#00000066"
    style.notmm_button_text.insensitive_color = "#00000019"
    style.notmm_button_text.hover_color = "#000000"
    style.notmm_button_text.size = 22
    ...
    style.rpa_button_text.size = 23
    style.rpa_active_button_text.size = 25
    ...
    if renpy.has_label('jumper'):
        style.gm_nav_frame.background = "ui/bg-gm-mod2.png" # android [str]
    else:
        style.gm_nav_frame.background = "ui/bg-gm.png"

    style.gm_nav_frame.top_padding = 52
    ...
    style.gm_nav_button_text.size = 27
    ...
    style.prefs_button_text.size = 22
