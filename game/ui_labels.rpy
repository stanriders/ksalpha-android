...
label splashscreen:
    ...
    play movie vid_disc
    python:
        ui.timer(15.85, ui.jumps('splashscreen_4ls')) #15.85
        ui.interact()
    return
...
label quit_from_os:
    ...
    #$ renpy.jump("confirm_quit") # naaah [str]
...
#custom preferences menu. Decidedly more custom than the rest.
    python:
        ...
        elif renpy.context()._main_menu and not coming_from_prefs_sub:
            renpy.transition(config.main_game_transition)
        else:
            renpy.transition(config.intra_transition)
        if renpy.context()._main_menu: # We'll draw a fake main menu for style reasons, or it'll just be boring black in the mm
        ...
        if gi_result == "return":
            if renpy.context()._main_menu:
                # for some reason _return no longer works in 6.18, using hacks [str]
                renpy.transition(config.game_main_transition)
                renpy.jump("_main_menu")
...
label language_screen:
    python:
        ...
        renpy.transition(config.intra_transition)
        if renpy.context()._main_menu: #renpy.context()._main_menu: # We'll draw a fake main menu for style reasons, or it'll just be boring black in the mm
            ui.image(style.mm_root.background)
        ui.image(style.gm_root.background)
        ui.image("ui/bg-config.png", xalign=0.5, yalign=0.5)
        layout.navigation(None)
...
label language_runamings_screen:
    python:
        ...
        renpy.transition(config.intra_transition)
        if renpy.context()._main_menu: #renpy.context()._main_menu:
            ui.image(style.mm_root.background)
        ui.image(style.gm_root.background)
    ...
    python:
        ...
        if fn == "return":
            if renpy.context()._main_menu:
                ...
                #ui.jumps("main_menu", "game_main_transition") #renpy.jump("_return")
                #ui.interact()
            else:
                renpy.transition(config.intra_transition)
                renpy.jump(_game_menu_screen)
            ...
            if not renpy.context()._main_menu and playthroughflag:
