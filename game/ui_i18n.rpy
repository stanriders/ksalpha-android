...
# "雪の降りしきるある日、久夫の夢が叶おうとしたその時、心臓発作が彼を襲う"
    ...
    def named_config():
        ...
        config.game_menu = [
           ...
           ( "skipping", displayStrings.game_menu_skip, ui.jumps("_return_skipping"), 'config.allow_skipping and not renpy.context()._main_menu'), # and not renpy.context()._main_menu'
           ...
           ( "save", displayStrings.game_menu_save, ui.jumps("save_screen", "intra_transition"), 'playthroughflag and not renpy.context()._main_menu' ), # and not renpy.context()._main_menu'
           ( "load", displayStrings.game_menu_load, ui.jumps("load_screen", "intra_transition"), 'renpy.list_saved_games()'),
           ( "mainmenu", displayStrings.game_menu_main, ui.jumps("confirm_mm", "intra_transition"), 'not renpy.context()._main_menu' ), #  'not renpy.context()._main_menu'
